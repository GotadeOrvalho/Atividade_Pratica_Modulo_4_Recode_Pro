...
<title>Passagens Cadastradas</title>
<link
...
<body class="body">
	...
	<main class="mb-0 row  justify-content-center">
		<section class=" row col-10  container bg-main ">
			<div class="text-center  fs-1 letra mb-0 pt-3">Lista das
				passagens cadastradas</div>
			<a href="cadastropassagem.html"
				class="btn mb-2 botao11 col-2 text-white button fw-bolder "> Nova
				Passagem </a>
			<table class="table">
				<thead>
					<tr>
				...
				<tbody>
					<jstl:forEach items="${listaPassagens}" var="p">
						<tr>
							<td>${p.id_passagem}</td>
							<td>${p.fk_cliente_id_cliente.id_cliente}</td>
							<td>${p.fk_id_destino.id_destino}</td>
							<td>${p.num_voo}</td>
							<td>${p.assento}</td>
							<td>${p.data_hora}</td>
							<td>${p.valor}</td>
							<td>
								<div class="d-flex">
									<div class="d-flex">
										<a href="passagens-edit?id_passagem=${p.id_passagem}" class="mx-1"
											title="Editar"> <i class="ri-file-edit-line"><img
												src="./img/imagemeditar.png"></i>
										</a> <a href="passagens-delete?id_passagem=${p.id_passagem}" class="mx-1"
											title="Cancelar"
											onclick="return confirm('Deseja excluir a passagem de id ${p.id_passagem}.')">
